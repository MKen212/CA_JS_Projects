...
// Get Form, Amount and Result elements
const form = document.querySelector(".lorem-form");
const amount = document.getElementById("amount");
const result = document.querySelector(".lorem-text");

// Set input element max attribute
amount.max = text.length;
// console.log(amount.attributes.max);

// Add submit event
form.addEventListener("submit", (event) => {
  // console.log("Hello");  // CANNOT be seen as default behaviour is happening
  event.preventDefault();
  // console.log("Hello");  // CAN be seen now that defaults are prevented
  const value = parseInt(amount.value);
  // console.log(value, typeof value);

  // Check input
  if (isNaN(value) || value <= 0 || value > text.length) {
    // If input empty or <> array length show one random para
    const random = Math.floor(Math.random() * text.length);
    result.innerHTML = `<p class="result">${text[random]}</p>`;
  } else {
    // If input number of paras then get slice of array chosen length
    let tempText = text.slice(0, value);
    // console.log(tempText);
    // Loop over new array and get the results in HTML
    let tempTextHTML = tempText.map((item) => {
      return `<p class="result">${item}</p>`;
    }).join("");
    result.innerHTML = tempTextHTML;
  }
});
